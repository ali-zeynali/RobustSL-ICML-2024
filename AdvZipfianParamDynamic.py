...
trials = 1
__path_dir__ = "AdvZipfianDynamic"
for n in ns:
    for alpha in alphas:
        for idx, error in enumerate(errors):
            ...
            if n <= 10000:
                ...
                TestDS(None, queries,
                       "{2}/Treap_n{3}_e{0}_a{1}.json".format(int(error * 100), alpha, __path_dir__, n), overWrite=True)

            print("n: {1}, alpha: {0}, Making LA-BTree...".format(alpha, n))
            if n <= 10000:
                treap = Treap(initial_elements.copy(), frequencies=frequencies.copy(), log_priority=True)

                TestDS(treap, queries,
                       "{2}/LABTree_n{3}_e{0}_a{1}.json".format(int(error * 100), alpha, __path_dir__, n))
            else:
                TestDS(None, queries,
                       "{2}/LABTree_n{3}_e{0}_a{1}.json".format(int(error * 100), alpha, __path_dir__, n), overWrite=True)
            ...
            TestDS(rsl, queries,
                   "{2}/RSL_n{3}_e{0}_a{1}_P0.json".format(int(error * 100), alpha, __path_dir__, n))
            ...
            TestDS(rsl, queries,
                   "{2}/RSLP_n{3}_e{0}_a{1}_P0.json".format(int(error * 100), alpha, __path_dir__, n))
            ...
            if error == 0:
                ...
                print("n: {1}, alpha: {0}, Making balanced BST...".format(alpha, n))
                copyResult("{2}/BalBST_n{3}_e{0}_a{1}.json".format(int(0 * 100), alpha, __path_dir__, n),
                           "{2}/BalBST_n{3}_e{0}_a{1}.json".format(int(error * 100), alpha, __path_dir__, n))

                print("n: {1}, alpha: {0}, Making AVL Tree...".format(alpha, n))
                copyResult("{2}/AVL_n{3}_e{0}_a{1}.json".format(int(0 * 100), alpha, __path_dir__, n),
                           "{2}/AVL_n{3}_e{0}_a{1}.json".format(int(error * 100), alpha, __path_dir__, n))

                print("n: {1}, alpha: {0}, Making RedBlack Tree...".format(alpha, n))
                copyResult("{2}/RedBlack_n{3}_e{0}_a{1}.json".format(int(0 * 100), alpha, __path_dir__, n),
                           "{2}/RedBlack_n{3}_e{0}_a{1}.json".format(int(error * 100), alpha, __path_dir__, n))
